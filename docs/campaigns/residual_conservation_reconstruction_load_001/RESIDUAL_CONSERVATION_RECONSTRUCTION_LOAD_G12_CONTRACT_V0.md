...
The observer returns only those seven substantive coordinates.

The assembly step then derives:

REQUESTED_POSTURE_COORDINATES_ALL_RESOLVED
...
Derived by assembly:
REQUESTED_POSTURE_COORDINATES_ALL_RESOLVED = NO

Assembly law:

REQUESTED_POSTURE_COORDINATES_ALL_RESOLVED = YES
iff none of the seven substantive returned coordinates is
UNRESOLVED_FROM_CARRIER.

Otherwise:
REQUESTED_POSTURE_COORDINATES_ALL_RESOLVED = NO.

If and only if the independently reconstructed pair matches the bounded
control/ablation discrimination may the pressure establish:
