...
def parse_manifest(raw: bytes) -> dict[str, Any]:
    ...
    allowed = {
        ...
        "input_sha256", "principal_id", "model", "temperature", "max_tokens",
        "purpose",
    }
...
def fresh_authority_ids() -> tuple[str, str]:
    ...
    return (
        f"CELL002.CAP.{capability_nonce}",
        f"CELL002.APPROVAL.{approval_nonce}",
    )
...
def postapproval_revalidation_witness(
    ...
    return {
        ...
        "declared_input_sha256": manifest["input_sha256"],
...
def process_once(
    ...
    for request_path in list_request_paths(repo, remote_head, p):
        ...
        try:
            reviewed_prompt_bytes = git_show(repo, manifest["source_ref"], manifest["input_path"])
        except Exception as e:
