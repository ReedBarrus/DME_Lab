"""Field-level candidate settlement over one exact materialized result witness.

The caller supplies field dispositions and explicit settlement bases. This layer
preserves full successor/work-spec/materialized-unit/admission/consumption
lineage through settlement.

It does not decide semantic truth, qualify scientific standing, infer external
consequence, grant authority, execute work, or mutate Atlas.
"""
...
from src.observation.materialized_invocation_result_witness_v0 import WITNESS_TYPE
from src.runtime.local_authority_consumption_v0 import canonical_sha256


SETTLEMENT_TYPE = "MATERIALIZED_INVOCATION_RESULT_CANDIDATE_SETTLEMENT_V0"
...
class MaterializedResultSettlementError(ValueError):
    pass


def _required_text(value: Any, field: str) -> str:
    if not isinstance(value, str) or not value:
        raise MaterializedResultSettlementError(
...
def _validate_witness(witness: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if hashlib.sha256(raw_output_bytes).hexdigest() != retained["raw_output_sha256"]:
        raise MaterializedResultSettlementError(
            "source witness raw output digest mismatch"
        )
    ...
    witness_core = {
        ...
        "materialized_work_item_id": retained["materialized_work_item_id"],
        "materialized_unit_integrity_sha256": retained[
            "materialized_unit_integrity_sha256"
        ],
        "work_spec_id": retained["work_spec_id"],
        "work_spec_integrity_sha256": retained[
            "work_spec_integrity_sha256"
        ],
...
def build_materialized_candidate_settlement(
    *,
    ...
    """Record externally supplied field-level settlement over exact work."""
    ...
    if not isinstance(settlement_basis, Mapping):
        raise MaterializedResultSettlementError(
            "settlement_basis must be an object"
        )
    ...
    settlement_core = {
        ...
        "source_successor_id": witness["successor_id"],
        "source_successor_integrity_sha256": witness[
            "successor_integrity_sha256"
        ],
        "source_work_spec_id": witness["work_spec_id"],
        "source_work_spec_integrity_sha256": witness[
            "work_spec_integrity_sha256"
        ],
        "source_materialized_work_item_id": witness[
            "materialized_work_item_id"
        ],
        "source_materialized_unit_integrity_sha256": witness[
            "materialized_unit_integrity_sha256"
        ],
        "source_consumption_id": witness["consumption_id"],
        "source_atomic_admission_id": witness["atomic_admission_id"],
    ...
    return {
        ...
        **settlement_core,
        ...
        "claim_ceiling": (
            ...
            "exact materialized-work result witness using externally supplied "
            "dispositions and bases. It conserves exact successor, work-spec, "
            "materialized-unit, admission, consumption, attempt, and output "
            "lineage. It does not establish semantic truth, external consequence, "
