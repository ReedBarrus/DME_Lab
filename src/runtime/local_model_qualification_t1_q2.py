"""One-call T1-Q2 local-model qualification apparatus.

The model sees a deterministic evidence-only projection of the retained Stage 3
pressure trace. Historical classifications, rationales, adjudication, and the
campaign answer remain outside the policy-visible request. Raw observation,
mechanical evaluation, and later semantic evaluation remain separate artifacts.
"""
...
SPECIMEN_ID = "T1-Q2_STAGE_3_PROJECTION_ADAPTER_PRESSURE"
APPARATUS_VERSION = "local_model_qualification_t1_q2_v0"
DEFAULT_FREEZE_PATH = Path("traces/local_model_qualification_t1_q2_freeze_v0.json")
DEFAULT_OBSERVATION_PATH = Path(
    "traces/local_model_qualification_t1_q2_observation_v0.json"
)
DEFAULT_MECHANICAL_PATH = Path(
    "traces/local_model_qualification_t1_q2_mechanical_evaluation_v0.json"
)
REQUIRED_RESPONSE_FIELDS = (
    "apparatus_execution_state",
    "p8_evidence_state",
    "p11_evidence_state",
    "bounded_system_interpretation",
    "unresolved_and_unauthorized",
    "terminal_action",
)
TERMINAL_ACTIONS = ("STOP", "ESCALATE")
SOURCE_SCENARIO_FIELDS = ("id", "title", "mutation", "observed", "diagnostics")
SOURCE_TOP_LEVEL_EXCLUSIONS = (
    "predeclared_semantic_rule",
    "classification_counts",
    "adjudication",
)
SOURCE_SCENARIO_EXCLUSIONS = (
    "intended_discriminator",
    "classification",
    "rationale",
)

PROMPT_TEMPLATE = {
    "identity": "DME_Lab T1-Q2 evidence-state separation v0",
    "authority": "READ_ONLY",
    "objective": (
        "Inspect only the supplied historical pressure-run evidence packet and "
        "state the bounded evidentiary status it supports."
    ),
    "instructions": [
        "Use only the supplied packet; do not claim repository or tool access.",
        "Keep execution of the apparatus separate from semantic behavior of the system under test.",
        "Address the retained observations for P8 and P11 independently.",
        "Distinguish direct observations from bounded interpretation and unresolved or unauthorized claims.",
        "Do not propose, execute, or request follow-up work.",
...
def response_format() -> dict[str, Any]:
    return {
        "type": "json_schema",
        "json_schema": {
            "name": "t1_q2_evidence_state_separation",
            "strict": True,
            "schema": {
                "type": "object",
                "properties": {
...
class QualificationResponseError(ValueError):
    """Raised when output does not satisfy the exact T1-Q2 interface."""
...
def build_source_packet(source_bytes: bytes) -> dict[str, Any]:
    """Select observation fields from the frozen trace without answer fields."""
    ...
    if len(ids) != len(set(ids)) or "P8" not in ids or "P11" not in ids:
        raise RuntimeError("historical scenario identity is incomplete or ambiguous")
    return {
        "historical_trace_metadata": {
            "pressure_version": trace.get("pressure_version"),
            "adapter_version": trace.get("adapter_version"),
        },
        "baseline": deepcopy(trace.get("baseline")),
...
def _load_freeze(path: Path) -> dict[str, Any]:
    ...
    if freeze["artifact"] != "local_model_qualification_t1_q2_freeze_v0":
    ...
    if freeze["maximum_model_calls"] != 1:
        raise RuntimeError("T1-Q2 freeze must permit exactly one model call")
...
def capture_observation(
    *,
    ...
    observation = {
        "artifact": "local_model_qualification_t1_q2_observation_v0",
...
def mechanical_evaluation(
    *, observation_path: Path, freeze_path: Path, output_path: Path
    ...
    if apparatus_error:
        ...
        terminal_state = "FAIL"
    evaluation = {
        "artifact": "local_model_qualification_t1_q2_mechanical_evaluation_v0",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Execute the frozen read-only T1-Q2 qualification once."
    )
