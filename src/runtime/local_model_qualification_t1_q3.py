"""One-call T1-Q3 local-model qualification apparatus.

The model sees the frozen Stage 4C classification rule and evidence observations,
but not expected discriminators, classifications, counts, rationales, campaign
answers, or adjudication. Observation, mechanical evaluation, and semantic
...
SPECIMEN_ID = "T1-Q3_STAGE_4C_INDEPENDENT_REPRESSURE"
APPARATUS_VERSION = "local_model_qualification_t1_q3_v0"
DEFAULT_FREEZE_PATH = Path("traces/local_model_qualification_t1_q3_freeze_v0.json")
DEFAULT_OBSERVATION_PATH = Path(
    "traces/local_model_qualification_t1_q3_observation_v0.json"
)
DEFAULT_MECHANICAL_PATH = Path(
    "traces/local_model_qualification_t1_q3_mechanical_evaluation_v0.json"
)
REQUIRED_RESPONSE_FIELDS = (
    "apparatus_execution_state",
    "named_specimen_result",
    "strongest_bounded_result",
    "explicit_residue_and_nonclaims",
    "authority_and_next_action_boundary",
    "terminal_action",
)
TERMINAL_ACTIONS = ("STOP", "ESCALATE")
SOURCE_SCENARIO_FIELDS = ("id", "title", "mutation", "observed", "diagnostics")
SOURCE_TOP_LEVEL_EXCLUSIONS = ("classification_counts", "adjudication")
...
PROMPT_TEMPLATE = {
    "identity": "DME_Lab T1-Q3 evidence-state separation v0",
    "authority": "READ_ONLY",
    "objective": (
        "Inspect only the supplied historical re-pressure evidence packet and "
        "report the strongest bounded result it supports."
    ),
    "instructions": [
        "Use only the supplied packet; do not claim repository or tool access.",
        "Apply the supplied classification rule to the named observations without inventing missing evidence.",
        "Keep apparatus execution separate from semantic behavior of the system under test.",
        "State explicit residue and nonclaims supported by the bounded evidence.",
        "Do not propose, execute, request, or authorize follow-up work.",
        "Use terminal_action STOP when the separation is complete.",
...
def response_format() -> dict[str, Any]:
    return {
        "type": "json_schema",
        "json_schema": {
            "name": "t1_q3_evidence_state_separation",
            "strict": True,
            "schema": {
                "type": "object",
                "properties": {
...
class QualificationResponseError(ValueError):
    """Raised when output does not satisfy the exact T1-Q3 interface."""


def parse_response(raw_model_response: str) -> dict[str, str]:
    """Parse the exact response shape without semantic repair."""
    ...
    if type(terminal_action) is not str or terminal_action not in TERMINAL_ACTIONS:
        raise QualificationResponseError("terminal_action must be STOP or ESCALATE")
    return {field: parsed[field] for field in REQUIRED_RESPONSE_FIELDS}


def build_source_packet(source_bytes: bytes) -> dict[str, Any]:
    """Select classification method and observations without answer fields."""

    try:
        trace = json.loads(source_bytes.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise RuntimeError("historical source trace is not valid UTF-8 JSON") from exc
    classification_rule = trace.get("predeclared_classification")
    scenarios = trace.get("scenarios")
    if not isinstance(classification_rule, dict) or not classification_rule:
        raise RuntimeError("historical source trace has no classification rule")
    if not isinstance(scenarios, list) or not scenarios:
        raise RuntimeError("historical source trace has no scenario observations")
    selected_scenarios: list[dict[str, Any]] = []
    for scenario in scenarios:
        if not isinstance(scenario, dict) or any(
            field not in scenario for field in SOURCE_SCENARIO_FIELDS
        ):
            raise RuntimeError("historical scenario lacks a required observation field")
        selected_scenarios.append(
            {field: deepcopy(scenario[field]) for field in SOURCE_SCENARIO_FIELDS}
        )
    ids = [scenario["id"] for scenario in selected_scenarios]
    if ids != [f"R{number}" for number in range(1, 13)]:
        raise RuntimeError("historical scenario identity or ordering is unexpected")
    return {
        "historical_trace_metadata": {
            "repressure_version": trace.get("repressure_version"),
            "adapter_version": trace.get("adapter_version"),
        },
        "classification_rule": deepcopy(classification_rule),
        "scenario_observations": selected_scenarios,
    }


def source_packet_hash(source_packet: Mapping[str, Any]) -> str:
    return _sha256_json(source_packet)


def _load_freeze(path: Path) -> dict[str, Any]:
    freeze = json.loads(path.read_text(encoding="utf-8"))
    if freeze["artifact"] != "local_model_qualification_t1_q3_freeze_v0":
    ...
    if freeze["maximum_model_calls"] != 1:
        raise RuntimeError("T1-Q3 freeze must permit exactly one model call")
...
def build_policy_visible_input(
    freeze: Mapping[str, Any], source_packet: Mapping[str, Any]
) -> dict[str, Any]:
    return {
        ...
        "source_packet": {
            ...
            "raw_source_sha256": freeze["source_specimen"]["sha256"],
            "selected_packet_sha256": freeze["source_specimen"][
                "selected_packet_sha256"
            ],
            "content": deepcopy(dict(source_packet)),
        },
    }
...
def capture_observation(
    *,
    ...
    selected_packet = build_source_packet(source_bytes)
    observed_packet_hash = source_packet_hash(selected_packet)
    if observed_packet_hash != freeze["source_specimen"]["selected_packet_sha256"]:
        raise RuntimeError("selected source packet does not match frozen hash")

    policy_input = build_policy_visible_input(freeze, selected_packet)
    ...
    observation = {
        "artifact": "local_model_qualification_t1_q3_observation_v0",
        ...
        "execution_basis": {
            ...
            "valid": resolved_execution_basis == current_head,
        },
        "source_specimen": {
    ...
    *, observation_path: Path, freeze_path: Path, output_path: Path
    ...
    try:
        ...
        request_reconstructs = (
            ...
            == [{"role": "user", "content": observation["serialized_policy_visible_input"]}]
    ...
    checks = {
        ...
        "declared_response_shape_satisfied": parsed_response is not None,
        "raw_interaction_retained_and_reconstructable": (
            observation_path.exists() and request_reconstructs and response_reconstructs
        ),
        "raw_source_basis_matched": (
            observation["source_specimen"]["sha256"]
            == observation["source_specimen"]["observed_sha256"]
        ),
        "selected_source_basis_matched": (
            observation["source_specimen"]["selected_packet_sha256"]
            == observation["source_specimen"]["observed_selected_packet_sha256"]
        ),
        ...
        "terminal_action_valid": (
            parsed_response is not None
            and parsed_response["terminal_action"] in TERMINAL_ACTIONS
        ),
    ...
    apparatus_error = not checks["inference_completed"]
    if apparatus_error:
        terminal_state = "APPARATUS_ERROR"
    elif all(checks.values()) and parsed_response is not None:
        terminal_state = (
            "ESCALATED" if parsed_response["terminal_action"] == "ESCALATE" else "PASS"
        )
    else:
        terminal_state = "FAIL"
    evaluation = {
        "artifact": "local_model_qualification_t1_q3_mechanical_evaluation_v0",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Execute the frozen read-only T1-Q3 qualification once."
    )
