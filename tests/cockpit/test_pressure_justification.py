...
class PressureJustificationTests(unittest.TestCase):
    ...
    def test_invocation_result_witness_is_selected_before_loop(self):
        workcycle = workcycle_fixture("AUTO_CONTINUATION_PRESSURE")
        workcycle["campaign_progress"]["T7"] = {"posture": "BOUNDED_PASS"}
        result = build_pressure_justification(
            workcycle=workcycle,
            ...
            qualification=qualification_fixture(
                self_moving=[
                    "INVOCATION_RESULT_WITNESS:UNFROZEN",
                    "REPEATED_METABOLIC_LOOP:UNFROZEN",
                ],
            ),
        )
    ...
    def test_invocation_result_settlement_is_selected_before_loop(self):
        workcycle = workcycle_fixture("AUTO_CONTINUATION_PRESSURE")
        workcycle["campaign_progress"]["T7"] = {"posture": "BOUNDED_PASS"}
        result = build_pressure_justification(
            workcycle=workcycle,
            ...
            qualification=qualification_fixture(
                self_moving=[
                    "INVOCATION_RESULT_SETTLEMENT:UNFROZEN",
                    "REPEATED_METABOLIC_LOOP:UNFROZEN",
                ],
            ),
        )
    ...
    def test_settlement_consequence_reconciliation_is_selected_before_loop(self):
        workcycle = workcycle_fixture("AUTO_CONTINUATION_PRESSURE")
        workcycle["campaign_progress"]["T7"] = {"posture": "BOUNDED_PASS"}
        result = build_pressure_justification(
            workcycle=workcycle,
            ...
            qualification=qualification_fixture(
                self_moving=[
                    "SETTLEMENT_CONSEQUENCE_RECONCILIATION:UNFROZEN",
                    "REPEATED_METABOLIC_LOOP:UNFROZEN",
                ],
            ),
        )
    ...
    def test_second_successor_is_selected_before_repeated_loop(self):
        workcycle = workcycle_fixture("AUTO_CONTINUATION_PRESSURE")
        workcycle["campaign_progress"]["T7"] = {"posture": "BOUNDED_PASS"}
        result = build_pressure_justification(
            workcycle=workcycle,
            ...
            qualification=qualification_fixture(
                self_moving=[
                    "SECOND_SUCCESSOR_FROM_RECONCILIATION:UNFROZEN",
                    "REPEATED_METABOLIC_LOOP:UNFROZEN",
                ],
            ),
        )
    ...
    def test_successor_materialization_is_selected_before_repeated_loop(self):
        workcycle = workcycle_fixture("AUTO_CONTINUATION_PRESSURE")
        workcycle["campaign_progress"]["T7"] = {"posture": "BOUNDED_PASS"}
        result = build_pressure_justification(
            workcycle=workcycle,
            ...
            qualification=qualification_fixture(
                self_moving=[
                    "SUCCESSOR_WORK_UNIT_MATERIALIZATION:UNFROZEN",
                    "REPEATED_METABOLIC_LOOP:UNFROZEN",
                ],
            ),
        )
    ...
    def test_materialized_unit_authority_binding_is_selected_before_loop(self):
        workcycle = workcycle_fixture("AUTO_CONTINUATION_PRESSURE")
        workcycle["campaign_progress"]["T7"] = {"posture": "BOUNDED_PASS"}
        result = build_pressure_justification(
            workcycle=workcycle,
            ...
            qualification=qualification_fixture(
                self_moving=[
                    "MATERIALIZED_UNIT_AUTHORITY_BINDING:UNFROZEN",
                    "REPEATED_METABOLIC_LOOP:UNFROZEN",
                ],
            ),
        )
    ...
    def test_materialized_consumption_is_selected_before_repeated_loop(self):
        workcycle = workcycle_fixture("AUTO_CONTINUATION_PRESSURE")
        workcycle["campaign_progress"]["T7"] = {"posture": "BOUNDED_PASS"}
        result = build_pressure_justification(
            workcycle=workcycle,
            ...
            qualification=qualification_fixture(
                self_moving=[
                    "MATERIALIZED_ADMITTED_AUTHORITY_CONSUMPTION:UNFROZEN",
                    "REPEATED_METABOLIC_LOOP:UNFROZEN",
                ],
            ),
        )
        ...
        self.assertEqual(
            result["proposed_pressure"],
            "MATERIALIZED_ADMITTED_AUTHORITY_CONSUMPTION_PRESSURE",
        )


    def test_materialized_result_witness_is_selected_before_repeated_loop(self):
        workcycle = workcycle_fixture("AUTO_CONTINUATION_PRESSURE")
        workcycle["campaign_progress"]["T7"] = {"posture": "BOUNDED_PASS"}
        result = build_pressure_justification(
            workcycle=workcycle,
            ...
            qualification=qualification_fixture(
                self_moving=[
                    "MATERIALIZED_INVOCATION_RESULT_WITNESS:UNFROZEN",
                    "REPEATED_METABOLIC_LOOP:UNFROZEN",
                ],
            ),
        )
    ...
    def test_materialized_result_settlement_is_selected_before_repeated_loop(self):
        workcycle = workcycle_fixture("AUTO_CONTINUATION_PRESSURE")
        workcycle["campaign_progress"]["T7"] = {"posture": "BOUNDED_PASS"}
        result = build_pressure_justification(
            workcycle=workcycle,
            ...
            qualification=qualification_fixture(
                self_moving=[
                    "MATERIALIZED_INVOCATION_RESULT_SETTLEMENT:UNFROZEN",
                    "REPEATED_METABOLIC_LOOP:UNFROZEN",
                ],
            ),
        )
