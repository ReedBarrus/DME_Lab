...
class AcousticSourceSidePositionalReturnPressureTest(unittest.TestCase):
    def test_retained_trace_preserves_insufficient_result_and_contamination(self) -> None:
        trace = json.loads(TRACE_PATH.read_text(encoding="utf-8"))
        self.assertEqual(
            trace["status"], "source_side_positional_return_evidence_insufficient"
        )
        self.assertEqual(
        ...
        self.assertFalse(trace["adjudication"]["source_stability"]["gate_passed"])
        self.assertFalse(
        ...
        self.assertEqual(len(trace["preflight"]["attempts"]), 2)
        self.assertEqual(
            [item["evaluation"]["sufficient_to_freeze"] for item in trace["preflight"]["attempts"]],
            [False, True],
        )
        self.assertEqual(len(trace["operator_post_acquisition_reports"]), 1)
        self.assertIn(
            "game audio",
            trace["operator_post_acquisition_reports"][0]["report"],
        )
        self.assertEqual(len(trace["process_failures"]), 1)
        self.assertFalse(trace["process_failures"][0]["sound_emitted"])
        self.assertTrue(
            all(
    ...
    def test_protocol_reuses_frozen_pr016_measurement_and_thresholds(self) -> None:
        ...
        self.assertEqual(protocol["intervention_locus"], "physical_speaker_position")
        self.assertEqual(PRIMARY_AMPLITUDE, 0.02)
        self.assertIn("microphone_position", POSITION_DEFINITIONS["fixed"])

    def test_synthetic_source_side_return_is_recurrent(self) -> None:
        result = evaluate_source_side_primary_blocks(_blocks())
        self.assertEqual(result["status"], "source_side_positional_return_recurrent")
    ...
    def test_B_like_return_is_observed_not_recurrent(self) -> None:
        result = evaluate_source_side_primary_blocks(_blocks(a2_like_a=False))
        self.assertEqual(
            result["status"], "source_side_position_observed_not_recurrent"
        )
    ...
    def test_nondiscriminated_B_gets_exact_status(self) -> None:
        ...
        self.assertEqual(result["status"], "source_side_position_not_discriminated")
    ...
    def test_capture_failure_is_preserved_as_insufficient(self) -> None:
        ...
        result = evaluate_source_side_primary_blocks(blocks)
        self.assertEqual(
            result["status"], "source_side_positional_return_evidence_insufficient"
        )
        self.assertEqual(result["failure_count"], 1)

    def test_source_instability_is_evidence_insufficient(self) -> None:
        blocks = _blocks()
