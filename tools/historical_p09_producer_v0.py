...
def _scope_core(scope: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "unit_ids": sorted(scope["unit_ids"]),
        "event_count": scope["event_count"],
        "event_log_sha256": scope["event_log_sha256"],
    }

def _invalid(cell_id: str, reason: str) -> dict[str, Any]:
    return {
...
def evaluate_cell(cell_id: str, cell: dict[str, Any]) -> dict[str, Any]:
    if set(cell) != {"scope", "events"}:
        raise P09Error("cell fields must be exact")

    scope = cell["scope"]
    events = cell["events"]
    if not isinstance(scope, dict) or set(scope) != {
        "scope_id", "unit_ids", "event_count", "event_log_sha256", "scope_identity"
    }:
    ...
    if not isinstance(scope["scope_identity"], str) or not scope["scope_identity"].startswith("sha256:"):
        raise P09Error("scope_identity malformed")

    if scope["scope_identity"] != _sha256_json(_scope_core(scope)):
        return _invalid(cell_id, "SCOPE_IDENTITY_MISMATCH")
    if scope["event_count"] != len(events):
        return _invalid(cell_id, "EVENT_COUNT_MISMATCH")
    if scope["event_log_sha256"] != _sha256_json(events):
        return _invalid(cell_id, "EVENT_LOG_IDENTITY_MISMATCH")

    state = {
        u: {"latest_start": None, "latest_terminal": None, "latest_independent": None}
        for u in units
    }
    ...
    for event in events:
        if not isinstance(event, dict) or set(event) != {
            "scope_id", "unit_id", "event_index", "event_type"
        }:
            raise P09Error("event fields must be exact")
        if event["scope_id"] != scope_id:
            return _invalid(cell_id, "EVENT_SCOPE_MISMATCH")
        u = event["unit_id"]
        et = event["event_type"]
        idx = event["event_index"]
        if u not in state:
            return _invalid(cell_id, "EVENT_UNIT_OUTSIDE_SCOPE")
        if et not in EVENTS:
            raise P09Error("unknown event_type")
        if not isinstance(idx, int) or idx != expected_index:
            return _invalid(cell_id, "EVENT_ORDER_NOT_CONTIGUOUS")
        expected_index += 1
    ...
    for u, posture in state.items():
        ...
        if start is None:
            continue
        closing = max(
            posture["latest_terminal"] if posture["latest_terminal"] is not None else -1,
            posture["latest_independent"] if posture["latest_independent"] is not None else -1,
        )
        if closing <= start:
            unfinished.append(u)
    ...
    return {
        ...
        "relation_type": RELATION_TYPE,
        "standing": standing,
        "basis_ref": f"scope://{scope_id}/{scope['scope_identity']}",
        "producer": PRODUCER,
        "version": VERSION,
